...
import time
import base64
# Compatibility shim: some versions of `streamlit_drawable_canvas` call
# `streamlit.elements.image.image_to_url`, which doesn't exist in newer
# Streamlit releases. Provide a lightweight implementation before
# importing `st_canvas` so the package can call it without failing.
try:
    import streamlit.elements.image as _st_image_mod
except Exception:
    _st_image_mod = None

if _st_image_mod is not None and not hasattr(_st_image_mod, "image_to_url"):
    import io, base64

    def image_to_url(img, width=None, use_container_width=False, fmt="RGB", ftype="PNG", key=None):
        """Return a data URL for a PIL Image.

        This is a minimal replacement for Streamlit's internal helper used by
        `streamlit_drawable_canvas`. It writes the image to a bytes buffer and
        returns a data URL (data:image/png;base64,...).
        """
        buf = io.BytesIO()
        if getattr(img, "mode", None) and img.mode != fmt:
            img = img.convert(fmt)
        img.save(buf, format=ftype)
        b64 = base64.b64encode(buf.getvalue()).decode("utf-8")
        return f"data:image/{ftype.lower()};base64,{b64}"

    setattr(_st_image_mod, "image_to_url", image_to_url)

# Now import the canvas helper (after the shim is available)
from streamlit_drawable_canvas import st_canvas
import numpy as np
# erased duplicate direct import; `erase_foreground` is already exported from `services`
...
def main():
    ...
    with tabs[0]:
        ...
        with col2:
            ...
            use_local = st.checkbox("Use local diffusion (requires diffusers & torch)", value=False, help="Generate images locally using installed diffusion models instead of the remote API")
        ...
        if st.button("🎨 Generate Images", type="primary"):
            ...
            with st.spinner("🎨 Generating your masterpiece..."):
                try:
                    ...
                    if use_local:
                        # Local generation path (diffusers). This will raise a helpful error if deps are missing.
                        result = generate_hd_local(
        ...
        if uploaded_file:
            ...
            with col1:
                ...
                with col_b:
                    ...
                    use_local_inpaint = st.checkbox("Use local inpainting (requires diffusers & torch)", value=False, help="Perform inpainting locally using installed diffusion models")
                    ...
                    with st.spinner("🎨 Generating..."):
                        try:
                            if use_local_inpaint:
                                # Local inpainting using diffusers (will raise a helpful message if deps missing)
                                result = inpaint_local(
                                    image_bytes,
                                    mask_bytes,
                                    prompt,
