...
def generative_fill(
    ...
    try:
        ...
        # If we got a normalized set of urls, return that, else return raw response
        return normalized if normalized else resp_json
    except requests.HTTPError as he:
        # Try to give a helpful message with body if available
        try:
            body = response.text
        except Exception:
            body = str(he)
        raise Exception(f"Generative fill HTTP error: {str(he)} - {body}")
