...
## 🧠 Local diffusion (optional)

If you want to run generation or inpainting locally (no external API), install the heavy dependencies and model weights. This is optional.

CPU-only example (may be slow):

```powershell
pip install "diffusers[torch]" transformers accelerate torch --extra-index-url https://download.pytorch.org/whl/cpu
```

GPU example (pick the correct torch wheel for your CUDA version):

```powershell
# Example for CUDA 11.8 (adjust per your system)
pip install torch --index-url https://download.pytorch.org/whl/cu118
pip install diffusers[torch] transformers accelerate
```

Once installed, enable the local options in the UI:
- "Use local diffusion" on the Generate Images tab
- "Use local inpainting" on the Generative Fill tab

## 🧪 Testing and CI

- Unit tests are in `tests/test_services.py` and can be run with:

```powershell
D:/adsnap-studio/.venv/Scripts/python.exe -m pytest -q
```
